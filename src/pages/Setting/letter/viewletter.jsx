...
// import Edit_freqs from './addcargoverns';
...
function Viewletter() {
  ...
  const Cargovefn = useCallback(async () => {
    try {
      const { success, data } = await api('GET', `mains/cargoverns`);
      if (!success) {
        DangerMsg('اشعارات التصنيفات', 'خطأ في تحميل البيانات');
        return;
      }
  ...
  const columns = [
    ...
    { field: 'value', headerName: ' الحرف ', width: 300 },
  ];
    ...
    <>
      <div>
        <Stack alignItems="center" mb={1}>
          <Typography variant="h3">واجهة الاحرف</Typography>
        </Stack>
